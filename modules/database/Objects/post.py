from . import combine_kwargs,db_run
...
class Post:
    ...
    def search(limit:int=32,order:str='created_at',**kwargs) -> list[dict[str,Any]]:
        ...
        LOOKUP = {
            ...
            "creator_names"   : (list[int],"MATCH (n)-[:Created]->(user:User) WHERE user.name IN $creator_names"),
        ...
        query += combine_kwargs(LOOKUP,kwargs)
        query += f"{DATA_QUERY} ORDER BY {ORDERS[order]} LIMIT $limit"
